<template>
  ...
  <div class="phonebind">
    ...
    <div class="common-div">
      <input type="tel" name="ems" maxlength="11" v-model="verificationCode" placeholder="请输入验证码"/>
    ...
    <div class="reset-btn" @click="_submit()">提交</div>
    ...
    methods: {
      ...
      _submit(){
           if(!this.phone){
                ...
                //验证手机
                api.edit_phone(this.phone,this.verificationCode).then(res => {
                    console.log(res)
                    if (res.Code == "0") {
                         this.$toast(res.Msg);
                        //成功 this.$router.go(-2);
                          //  this.$router.replace('/Success');
                           this.$router.push({name:'Success',params:{phone:this.phone}})
                    } else {
                        this.$toast(res.Msg);
