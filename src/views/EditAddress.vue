  ...
  <div class="editaddress">
    ...
    <div class="common">
      <div class="common-div">
        <input type="text" name="username" v-model="username" placeholder="姓名" required />
      </div>
      <div class="common-div">
        <input type="tel" name="user_phone" v-model="user_phone" placeholder="手机号" required/>
      </div>
      <div class="common-div">
        <input type="tel" name="postalcode" v-model="postalcode" placeholder="邮政编码" required/>
      </div>
      <div class="common-div">
        <input type="text" style="width: 100%"  @click="popAlert()"  v-model="province_city" placeholder="请选择省市区" required readonly=""/>
...
//  import { address, slots } from '../util/address.js';
  ...
  export default {
    ...
    methods: {
      ...
      loadAddress(address_id) {
        api.idaddress(address_id).then(res => {
          console.log(res)
          if (res.Code == "0") {
            //成功t
            this.username=res.Data.recipients;
            this.user_phone=res.Data.telephone;
            this.postalcode=res.Data.postalcode;
            this.province_city=res.Data.big_address;
            this.small_address=res.Data.small_address;
            if(res.Data.is_default=="1"){
              this.value.push('设置默认地址');
            }else{
              this.value=[];
            }
          } else {
      ...
      addSave(){
        ...
        api.editaddress(params).then(res => {
          console.log(res)
          if (res.Code == "0") {
            //成功
            //成功
...
<style scoped>
  /*html,body,html body #app {height: 100%;}*/
  .editaddress {
    font-size: 13px;
    width: 100%;
    height: 100%;
...
.mint-cell-wrapper,  .mint-cell:last-child  {
  ...
  .editaddress  .mint-cell .mint-cell-wrapper{
    height: 100px;
    margin-top: 5px;
    font-size: 25px;
  }
  .editaddress  .mint-checkbox-core{
      width: 30px;
     height: 30px;
